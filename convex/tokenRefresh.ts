...
/**
 * INTERNAL ACTION: Refresh expired Twitter/X OAuth tokens.
 *
 * This action refreshes Twitter access and refresh tokens using the X OAuth 2.0
 * token refresh endpoint. It implements retry logic with exponential backoff for transient
 * failures and timeout handling.
 *
 * Twitter token expiration:
 * - Access tokens: 2 hours (7,200 seconds)
 * - Refresh tokens: Does not expire (rolling expiration)
 *
 ...
 */
export const refreshTwitterToken = internalAction({
  args: {
    userId: v.string(),
  },
  returns: v.object({
  ...
  handler: async (
    ...
    try {
      ...
      // Call Twitter token refresh endpoint with retry logic
      ...
      for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
          ...
          if (!response.ok) {
            ...
            if (response.status >= 500 || response.status === 429) {
              if (attempt < maxRetries - 1) {
                ...
                console.warn(
                  `Twitter token refresh failed (attempt ${attempt + 1}/${maxRetries}): ${response.statusText}. Retrying in ${delayMs}ms...`,
                );
            ...
            throw new Error(
              `Twitter token refresh failed: ${response.statusText} - ${errorText}`,
            );
          }
          ...
          if (!data.access_token || !data.refresh_token) {
            throw new Error(
              "Twitter token refresh response missing required tokens",
            );
          }

          // Calculate new expiration timestamp
          const expiresIn = data.expires_in || 7200; // Default: 2 hours in seconds
          ...
          console.log(`[Token Refresh] Successfully refreshed Twitter token for user ${args.userId}`);
          ...
          if (error instanceof Error && error.name === "AbortError") {
            if (attempt < maxRetries - 1) {
              ...
              console.warn(
                `Twitter token refresh timed out (attempt ${attempt + 1}/${maxRetries}). Retrying in ${delayMs}ms...`,
              );
              await new Promise((resolve) => setTimeout(resolve, delayMs));
              continue;
            }
            lastError = new Error(
              "Twitter token refresh timed out after multiple attempts",
            );
            break;
          }
          ...
          if (attempt < maxRetries - 1) {
            const delayMs = Math.pow(2, attempt) * 1000;
            console.warn(
              `Twitter token refresh error (attempt ${attempt + 1}/${maxRetries}): ${error instanceof Error ? error.message : "Unknown error"}. Retrying in ${delayMs}ms...`,
            );
            await new Promise((resolve) => setTimeout(resolve, delayMs));
            lastError =
      ...
      return {
        ...
        error:
          lastError?.message ||
          "Twitter token refresh failed after all retries",
        needsReauth: false,
      };
    } catch (error) {

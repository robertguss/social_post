...
**IMPORTANT**: This story focuses on establishing the OAuth flow and storing tokens. **Token encryption** is explicitly deferred to Story 1.3. For this story, tokens will be stored as plain text in the `user_connections` table, which is acceptable for development but must be addressed before production deployment.
...
**Token Storage (for this story):**

- Tokens will be stored as plain text in the `user_connections` table
- **Token encryption is deferred to Story 1.3**
- Mark this as a known security limitation in implementation notes
...
- **IMPORTANT NOTE**: OAuth tokens are stored as plain text in this story as per requirements. Token encryption will be implemented in Story 1.3.

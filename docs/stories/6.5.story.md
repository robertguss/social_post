...
- [x] Task 1: Define `user_preferences` schema in `convex/schema.ts` (AC: 3)
  - [x] Add `user_preferences` table with fields: `clerkUserId`, `platform`, `dayOfWeek`, `customTimeRanges`
  ...
  - [x] Document that times are stored in user's local timezone (different from recommendations table which uses UTC)

...
- [x] **Task 1: Create duplicate queue detection query** (AC: 1, 2)
  - [x] In `convex/queues.ts`, create query `checkDuplicateQueue`
  - [x] Args: `originalPostId: v.id("posts")`
  - [x] Verify user authentication
  - [x] Query recurring_queues for queues matching originalPostId and clerkUserId
  - [x] Filter results to include only "active" and "paused" queues (exclude "completed")
  - [x] Return array of duplicate queues (or null/empty if none found)
  - [x] Include queue status and nextScheduledTime in results

- [x] **Task 2: Update createQueue mutation to check duplicates** (AC: 1, 2, 3)
  - [x] In `convex/queues.ts`, modify `createQueue` mutation
  - [x] Before creating queue, call internal duplicate check logic
  - [x] If duplicate exists, throw error with code "DUPLICATE_QUEUE_EXISTS"
  - [x] Include duplicate queue details in error response (queueId, status)
  - [x] OR: Add optional `force: v.optional(v.boolean())` parameter to allow bypassing check
  - [x] If `force: true`, skip duplicate check and create queue anyway
  - [x] Document this behavior in mutation comments

- [x] **Task 3: Create conflict detection query** (AC: 4, 7)
  - [x] In `convex/queues.ts`, create query `detectSchedulingConflicts`
  - [x] Verify user authentication
  - [x] Fetch all user's active queues
  - [x] Fetch all user's scheduled posts (status "scheduled")
  - [x] For each queue, check if nextScheduledTime conflicts with any scheduled post
  - [x] Conflict definition: times are within 1 hour (3600000 ms) of each other AND posts target the same platform
  - [x] Return array of conflicts: `{ queueId, postId, queueTime, postTime, platform }`
  - [x] Handle dual-platform posts (check Twitter and LinkedIn separately)

- [x] **Task 4: Create ConflictWarning component** (AC: 2, 3)
  - [x] Create new file: `components/features/ConflictWarning.tsx`
  - [x] Accept props: `duplicateQueue` object (from duplicate check)
  - [x] Display warning banner with alert icon (orange/warning color)
  - [x] Message: "A queue for this post already exists."
  - [x] Display duplicate queue details: status, interval, next scheduled time
  - [x] Render three action buttons:
  ...
  - [x] Style with shadcn/ui Alert or custom banner component
  - [x] Make accessible (keyboard navigation, screen reader support)
...
- [x] **Task 6: Add conflict indicators to QueueCard** (AC: 4)
  - [x] In `components/features/QueueCard.tsx`, accept new prop: `conflicts: Conflict[]`
  - [x] If conflicts exist for this queue, display conflict warning icon/badge
  - [x] Use orange/warning color for conflict indicator
  - [x] Show tooltip on hover with conflict details: "Conflicts with scheduled post on {date}"
  - [x] Make conflict indicator clickable to open conflict resolution modal
  - [x] Add aria-label for accessibility: "This queue has scheduling conflicts"

- [x] **Task 7: Create ConflictResolutionModal component** (AC: 5)
...
- [x] **Task 8: Integrate conflict detection into QueueList** (AC: 4)
...
- [x] **Task 9: Implement exact conflict prevention** (AC: 6)
...
- [x] **Task 10: Add "View Existing Queue" navigation** (AC: 3)
...
- [x] **Task 11: Test duplicate queue detection** (AC: 1, 2)
...
- [x] **Task 12: Test conflict detection logic** (AC: 4, 6, 7)
...
- [x] **Task 15: Test exact conflict prevention** (AC: 6)
...
- Implemented all backend conflict detection queries (`checkDuplicateQueue`, `detectSchedulingConflicts`)
- Added duplicate queue check with optional `force` parameter to `createQueue` mutation
- Implemented exact conflict prevention helper function `checkExactConflict` with 1-second tolerance
- Integrated exact conflict validation into both `createQueue` and `updateQueue` mutations
- Created `ConflictWarning` component using shadcn AlertDialog for duplicate queue warnings
- Created `ConflictResolutionModal` component with time adjustment and conflict resolution options
- Enhanced `QueueCard` with conflict indicators, tooltips, and accessibility features
- Integrated conflict detection into `QueueList` with global conflict summary banner
- Implemented URL-based queue highlighting navigation (`/queues?highlight={queueId}`)
- Added comprehensive unit tests for all conflict detection logic (24 tests passing)
- Task 5 (Integrate duplicate check into queue creation flow) marked as pending - requires QueueCreateModal which was not found in codebase
- Tasks 13-14 (Component tests) marked as pending - follow same pattern as existing component tests when needed

### File List

**Modified Files:**

- convex/queues.ts (added 3 queries, modified 2 mutations, added helper function)
- convex/queues.test.ts (added 17 new test cases)
- components/features/QueueCard.tsx (added conflict indicator and props)
- components/features/QueueList.tsx (added conflict detection integration)

**New Files:**

- components/features/ConflictWarning.tsx
- components/features/ConflictResolutionModal.tsx

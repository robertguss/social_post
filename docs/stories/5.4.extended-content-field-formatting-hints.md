...
- [ ] **Task 1: Create LinkedInFormattingHints component** (AC: 1, 2, 5)
  - [ ] Create new component `components/features/LinkedInFormattingHints.tsx`
  - [ ] Define props: `content: string`, `isVisible: boolean`, `onDismiss: () => void`
  - [ ] Design subtle, non-intrusive hint panel (e.g., small card below LinkedIn text area)
  - [ ] Display static formatting hints:
    - "Use line breaks for readability"
    - "Add emojis for engagement"
    - "Include hashtags at the end"
  - [ ] Add minimize/dismiss button (small 'x' icon)
  - [ ] Use subdued styling (light background, small text, borderless or subtle border)
  - [ ] Ensure hints are collapsible and remember state (use localStorage or component state)

- [ ] **Task 2: Implement context-aware hint logic** (AC: 6)
  - [ ] Analyze `content` prop to determine which hints to show
  - [ ] Detect if content has hashtags (regex: `/#\w+/g`)
  - [ ] If content length > 100 characters AND no hashtags detected:
    - Show additional hint: "Consider adding relevant hashtags"
  - [ ] Detect if content has line breaks (check for `\n` characters)
  - [ ] If content length > 200 characters AND no line breaks:
    - Show additional hint: "Add line breaks to improve readability"
  - [ ] Make hints dynamic based on content analysis
  - [ ] Ensure hints update as user types (but not too frequently - consider debounce if needed)

- [ ] **Task 3: Create LinkedIn Tips popover** (AC: 3)
  - [ ] Add "Tips" button/icon near LinkedIn text area label or hint panel
  - [ ] Use shadcn/ui Popover component for tips display
  - [ ] Populate popover with LinkedIn best practices:
    ...
    - "Posts with 8-12 hashtags tend to perform well"
    - "Tag relevant people or companies to increase reach"
    - "Emoji can increase engagement, but use sparingly"
    - "Ask questions to encourage comments"
  - [ ] Style popover for readability (bulleted list, clear headings)
  - [ ] Ensure popover is accessible (keyboard navigable, proper ARIA attributes)
  - [ ] Position popover to not obstruct text area
...
- [ ] **Task 5: Integrate LinkedInFormattingHints into DualPlatformTextFields** (AC: 1, 2, 5, 6)
  - [ ] In `DualPlatformTextFields.tsx`, import `LinkedInFormattingHints` component
  - [ ] Add state to track hint panel visibility: `const [showLinkedInHints, setShowLinkedInHints] = useState(true);`
  - [ ] Render `<LinkedInFormattingHints>` component below LinkedIn text area
  - [ ] Pass `linkedInContent` as the `content` prop for context-aware hints
  - [ ] Pass visibility state and dismiss handler
  - [ ] Store hint visibility preference in localStorage: `linkedin-hints-visible`
  - [ ] On component mount, check localStorage to restore user's preference

- [ ] **Task 6: Ensure hints are informational only** (AC: 7)
  - [ ] Verify that hints do NOT block form submission
  - [ ] Verify that hints do NOT modify user content automatically
  - [ ] Ensure dismissing hints doesn't affect content or validation
  - [ ] All hints should use language like "Consider..." or "Tip:" to indicate they're suggestions
  - [ ] No "error" or "warning" styling - use neutral, helpful tone

- [ ] **Task 7: Add unit tests for LinkedInFormattingHints** (AC: 1-7)
  - [ ] Create test file `components/features/LinkedInFormattingHints.test.tsx`
  - [ ] Test: Component renders with default hints
  - [ ] Test: Dismiss button hides the hint panel
  - [ ] Test: Context-aware hashtag hint appears when content > 100 chars and no hashtags
  - [ ] Test: Context-aware line break hint appears when content > 200 chars and no line breaks
  - [ ] Test: Hints do not appear for short content
  - [ ] Test: Tips popover opens and closes correctly
  - [ ] Test: Markdown preview toggle works (if implemented)

- [ ] **Task 8: Add integration tests for hint panel** (AC: 5, 6, 7)
  - [ ] Test: Hint panel visibility is restored from localStorage on mount
  - [ ] Test: Dismissing hints persists preference in localStorage
  - [ ] Test: Hints update as user types in LinkedIn text area
  - [ ] Test: Form submission works regardless of hint visibility
  - [ ] Test: Hints never modify content automatically
...
const contextAwareHints: Hint[] = [
  {
    ...
    condition: (content) =>
      content.length > 100 && !/#\w+/.test(content)
  },
  {
    id: 'add-line-breaks',
...
const activeHints = contextAwareHints.filter((hint) =>

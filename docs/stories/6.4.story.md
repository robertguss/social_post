...
- [ ] Task 1: Define `post_performance` schema in `convex/schema.ts` (AC: 1)
  - [ ] Add `post_performance` table with fields: `postId`, `platform`, `publishedTime`, `engagementMetrics`, `fetchedAt`
  - [ ] Use Convex validators for type safety (`v.id("posts")` for postId, `v.object()` for engagementMetrics)
  - [ ] Define `engagementMetrics` structure: `{ likes: number, shares: number, comments: number, impressions?: number }`
  - [ ] Add index `by_post` on `["postId"]` for efficient post-specific lookups
  - [ ] Add index `by_platform_time` on `["platform", "publishedTime"]` for time-based analysis
  - [ ] Document schema fields with inline comments explaining future activation

- [ ] Task 2: Create stub Convex action for fetching engagement metrics (AC: 2)
  - [ ] Create `convex/analytics.ts` file for performance tracking logic
  - [ ] Add `"use node";` directive for Node.js runtime (required for external API calls)
  - [ ] Define action `fetchEngagementMetrics` with parameters: `postId`, `platform`
  - [ ] Implement stub logic that returns mock data or throws "not implemented" error
  - [ ] Add authentication check to ensure only the post owner can fetch metrics
  - [ ] Document required API endpoints for Twitter and LinkedIn engagement data
  - [ ] Add placeholder for API key environment variables (TWITTER_API_KEY, LINKEDIN_API_KEY)

- [ ] Task 3: Create mutation to store performance data (AC: 3)
  - [ ] Define mutation `storePerformanceData` in `convex/analytics.ts`
  - [ ] Accept parameters: `postId`, `platform`, `engagementMetrics`
  - [ ] Validate that postId exists and belongs to the authenticated user
  - [ ] Insert record into `post_performance` table with current timestamp as `fetchedAt`
  - [ ] Link to original post via `postId` (foreign key relationship)
  - [ ] Add error handling for duplicate entries (update existing if re-fetching metrics)

- [ ] Task 4: Create placeholder for scheduled metric fetching (AC: 2)
  - [ ] Define scheduled function stub `scheduledMetricsFetch` in `convex/analytics.ts`
  - [ ] Document intended schedule: daily or every 6 hours
  - [ ] Add logic to query all published posts from past 7 days
  - [ ] Loop through posts and call `fetchEngagementMetrics` action for each
  - [ ] Add rate limiting logic to respect API quotas (Twitter/LinkedIn rate limits)
  - [ ] Mark function as inactive (commented out or feature-flagged) until API access is configured

- [ ] Task 5: Enhance recommendation algorithm with historical performance weighting (AC: 4)
  - [ ] Modify `getRecommendedTimes` query in `convex/recommendations.ts`
  - [ ] Add optional logic to query `post_performance` table for user's historical data
  - [ ] Calculate average engagement by hour of day for each platform
  - [ ] Weight research-based engagement scores with historical performance factor
  - [ ] Example formula: `finalScore = (researchScore * 0.6) + (historicalScore * 0.4)`
  - [ ] Add feature flag to enable/disable historical weighting (default: disabled)
  - [ ] Document how weighting algorithm works in code comments

- [ ] Task 6: Create "Performance Insights" UI page (AC: 5)
  - [ ] Create new page `app/insights/page.tsx` for performance insights
  - [ ] Add route protection via Clerk middleware (authenticated users only)
  - [ ] Create Convex query to aggregate performance data by time of day
  - [ ] Display chart/graph showing best-performing posting times (use chart library like Recharts)
  - [ ] Show engagement metrics breakdown (likes, shares, comments)
  - [ ] Add filter by platform (Twitter vs LinkedIn)
  - [ ] Add filter by date range (last 7 days, 30 days, all time)
  - [ ] Show "Feature not yet active" message if no performance data exists

- [ ] Task 7: Document API requirements and setup steps (AC: 6, 7)
  - [ ] Create documentation file: `docs/features/performance-tracking.md`
  - [ ] Document Twitter/X API requirements for engagement metrics
  - [ ] Document LinkedIn API requirements for engagement metrics
  - [ ] List required OAuth scopes for accessing analytics data
  - [ ] Provide step-by-step setup guide for obtaining API credentials
  - [ ] Document rate limits and quota considerations for each platform
  - [ ] Include troubleshooting section for common API access issues
  - [ ] Add note about API pricing and potential costs

- [ ] Task 8: Add feature flag system for gradual activation (AC: 6)
  - [ ] Create feature flag configuration (e.g., in environment variables or Convex config)
  - [ ] Add `PERFORMANCE_TRACKING_ENABLED` flag (default: false)
  - [ ] Gate all performance tracking logic behind this flag
  - [ ] Show/hide "Performance Insights" nav link based on flag
  - [ ] Display setup instructions when flag is false but user tries to access feature
  - [ ] Add admin toggle to enable feature when API access is configured

- [ ] Task 9: Write unit tests for performance tracking logic (AC: 1-4)
  - [ ] Test `post_performance` schema validation
  - [ ] Test `storePerformanceData` mutation with valid and invalid data
  - [ ] Test engagement metrics calculation logic
  - [ ] Test historical weighting algorithm with mock data
  - [ ] Test feature flag behavior (enabled vs disabled states)

- [ ] Task 10: Write integration tests (AC: 2, 3)
  - [ ] Test full flow: fetch metrics → store data → aggregate insights
  - [ ] Test scheduled function logic (with mocked API responses)
  - [ ] Test authentication requirements for all actions/mutations
  - [ ] Test recommendation algorithm with historical performance data
  - [ ] Test "Performance Insights" page rendering with real data
...
export const fetchEngagementMetrics = action({
  ...
  handler: async (ctx, args) => {
    ...
    throw new Error("Performance tracking not yet implemented - API access required");
...
function calculateFinalScore(researchScore, historicalData, hourOfDay) {
  ...
  const postsAtThisHour = historicalData.filter(p => getHourOfDay(p.publishedTime) === hourOfDay);
  const avgEngagement = postsAtThisHour.reduce((sum, p) => sum + p.engagementMetrics.likes, 0) / postsAtThisHour.length;
...
// In code:
const isPerformanceTrackingEnabled = process.env.PERFORMANCE_TRACKING_ENABLED === "true";

if (!isPerformanceTrackingEnabled) {
  throw new Error("Performance tracking is not yet enabled. Configure API access first.");
}
```

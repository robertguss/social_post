...
- [ ] Task 1: Define `posting_time_recommendations` schema in `convex/schema.ts` (AC: 1, 3, 4, 5)
  - [ ] Add `posting_time_recommendations` table with fields: `platform`, `dayOfWeek`, `hourRanges`, `engagementScore`, `source`
  - [ ] Use Convex validators for type safety (`v.string()`, `v.array()`, etc.)
  - [ ] Define index `by_platform_day` on `["platform", "dayOfWeek"]`
  - [ ] Ensure all time ranges in `hourRanges` are stored in UTC format
  - [ ] Document the schema structure with inline comments

- [ ] Task 2: Create seed data mutation for initial recommendations (AC: 2, 6)
  - [ ] Create `convex/seedRecommendations.ts` with mutation to populate initial data
  - [ ] Add research-based best practice data for Twitter (e.g., Tue-Thu 9-11am EST)
  - [ ] Add research-based best practice data for LinkedIn (e.g., Wed 10am-12pm EST)
  - [ ] Convert all EST/local times to UTC timestamps in `hourRanges`
  - [ ] Include engagement scores (normalized 0-100) for each recommendation
  - [ ] Mark source as "industry research" for all seeded data
  - [ ] Add logic to prevent duplicate seeding (check if data already exists)

- [ ] Task 3: Create admin/utility function to run seed operation (AC: 6)
  - [ ] Create internal mutation `seedPostingTimeRecommendations` that can be called programmatically
  - [ ] Add authentication check to ensure only admin users can seed data
  - [ ] Add logging to track when seed operation completes
  - [ ] Document how to run the seed function in development vs production

- [ ] Task 4: Write unit tests for schema validation (AC: 1, 3, 4)
  - [ ] Test that `posting_time_recommendations` accepts valid data structures
  - [ ] Test that index lookups work correctly for `by_platform_day`
  - [ ] Test that multiple recommendations per platform/day combination are supported
  - [ ] Test timezone handling (UTC storage verification)
  - [ ] Test that invalid data structures are rejected by validators

- [ ] Task 5: Write integration tests for seed function (AC: 2, 6)
  - [ ] Test that seed function successfully populates the table
  - [ ] Test that seeded data contains Twitter and LinkedIn recommendations
  - [ ] Verify engagement scores are within expected range
  - [ ] Test that duplicate seeding is prevented
  - [ ] Verify all seeded times are in UTC format
